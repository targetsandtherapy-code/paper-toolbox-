...
def process_paper(
    ...
    def _process_one_marker(marker_idx, cid, marker):
        ...
        target_lang = rec_lang
        log(f"  ref_type: {ref_rt} | lang: {rec_lang}" + (f" — {_rth}" if _rth else ""))

        best = None
        max_search_rounds = 5

        # 政策文件/专著《…》：直接 DuckDuckGo 搜，不走学术库
        ...
        q = _base_cn if target_lang == "cn" else _base_en
        if not q:
            q = _base_en if target_lang == "cn" else _base_cn
        q = _ensure_nursing_query(q, target_lang, scope_nursing)
            ...
            if round_i < max_search_rounds - 1:
                try:
                    new_cn, new_en = analyzer.refine_search(
                        ...
                        target_lang=target_lang, paper_title=paper_title,
                    )
                    q = (new_cn if target_lang == "cn" else new_en) or ""
                    q = _ensure_nursing_query(q, target_lang, scope_nursing)
                    if not q:
                        q = (new_en if target_lang == "cn" else new_cn) or ""
                except Exception:
                    break
